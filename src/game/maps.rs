...
pub fn create_walls(physics: &mut PhysicsEngine, width: f32, height: f32) {
    ...
    for i in 0..5 {
        ...
        let restitution = if rng.gen_bool(0.5) { 3.0 } else { 5.0 };
        let user_data = if restitution > 4.0 { 1 } else { 3 }; // 1=Red, 3=Orange
        ...
        let restitution_right = if rng.gen_bool(0.5) { 3.0 } else { 5.0 };
        let user_data_right = if restitution_right > 4.0 { 1 } else { 3 };
    ...
    let collider = ColliderBuilder::cuboid(5.0, chute_height / 2.0)
        .translation(vector![-(exit_gap/2.0 + 5.0), bottom_y + chute_height/2.0])
        .friction(0.0)
        .collision_groups(InteractionGroups::new(super::GROUP_MAP, super::GROUP_BALL))
        .build();
    physics.collider_set.insert(collider);
...
pub fn create_pins(physics: &mut PhysicsEngine, width: f32, height: f32) {
    ...
    for r in 0..rows {
        for c in 0..cols {
             ...
             let mut rng = rand::thread_rng();
             if spinner_count < 5 && rng.gen_bool(0.05) { // 5% chance, max 5
                  spinner_count += 1;
                  let spinner_len = if rng.gen_bool(0.5) { 40.0 } else { 80.0 };
                  // Random speed: 1.0 to 5.0
                  let speed_mag = rng.gen_range(1.0..5.0);
                  let speed = if rng.gen_bool(0.5) { speed_mag } else { -speed_mag };
                  create_spinner(physics, x, y, spinner_len, speed);
                  continue;
             }
...
pub fn create_spinner(physics: &mut PhysicsEngine, x: f32, y: f32, length: f32, speed: f32) {
    // 1. Static Anchor (invisible or small)
    let anchor_rb = RigidBodyBuilder::fixed()
        .translation(vector![x, y])
        .build();
    ...
    physics.impulse_joint_set.insert(anchor_handle, blade_handle, joint, true);
}
...
pub fn create_seesaw(physics: &mut PhysicsEngine, x: f32, y: f32, width: f32) {
    ...
    let collider = ColliderBuilder::cuboid(width / 2.0, 3.0)
        ...
        .collision_groups(InteractionGroups::new(super::GROUP_MAP, super::GROUP_BALL))
        .build();
    physics.collider_set.insert_with_parent(collider, plank_handle, &mut physics.rigid_body_set);
    ...
    physics.impulse_joint_set.insert(pivot_handle, plank_handle, joint, true);
}
